...
describe('A HTTP Server', () => {
  ...
  describe('when GET /divide', () => {
    it('should respond with a status code of 200 and the payload value is division result of a and b correctly', async () => {
      ...
      const responseJson = JSON.parse(response.payload);
      expect(response.statusCode).toEqual(200);
      expect(responseJson.value).toEqual(1.5); // a / b
      expect(spyDivide).toBeCalledWith(a, b);
    });
  });

  describe('when GET /rectangle/perimeter', () => {
    it('should respond with a status code of 200 and the payload value is calculating the perimeter of the rectangle correctly', async () => {
      // Arrange
      const length = 12;
      const width = 8;
      ...
      const responseJson = JSON.parse(response.payload);
      expect(response.statusCode).toEqual(200);
      expect(responseJson.value).toEqual(40); // 2 * (length + width)
      expect(spyCalculateRectanglePerimeter).toBeCalledWith(length, width);
    });
  });

  describe('when GET /rectangle/area', () => {
    it('should respond with a status code of 200 and the payload value is calculating the area of the rectangle correctly', async () => {
      // Arrange
      const length = 12;
      const width = 8;
      const figureCalculator = new FigureCalculator(MathBasic);
      ...
      const responseJson = JSON.parse(response.payload);
      expect(response.statusCode).toEqual(200);
      expect(responseJson.value).toEqual(96); // (length * width)
      expect(spyCalculateRectangleArea).toBeCalledWith(length, width);
    });
  });
  describe('when GET /triangle/perimeter', () => {
    it('should respond with a status code of 200 and the payload value is calculating the perimeter of the triangle correctly', async () => {
      // Arrange
      const sideA = 12;
      const sideB = 8;
      const base = 10;
  ...
  describe('when GET /triangle/area', () => {
    it('should respond with a status code of 200 and the payload value is calculating the area of the triangle correctly', async () => {
      // Arrange
      const base = 10;
      const height = 12;
      const figureCalculator = new FigureCalculator(MathBasic);
